...
/*  Trabalho 1 - INF 492
    ...
    Sem paralelismo:
        g++ E4_SimulatedAnnealing.cpp -O3
*/
...
int cutValue(const vector <pair<int,int> > &edge, const vector<int>&f, int numVertices) {
    ...
    // The "heavier" part of the code was made a little bit litgher using parallelism
...
vector <int> nextNeighbor(const vector <pair<int,int> > &edge, vector <int>&f, int numVertices, int T) { // Less Random when temp is decreasing
    ...
    double tempFactor = T/(numVertices*numVertices*4.0); // Less Random when temp is decreasing
    int neighborhoodSize = (int) ceil(tempFactor*numVertices) + 2;
...
int main() {
    ...
    for (int i=0; i<numVertices; i++) f[i] = i + 1; //Initializing positions vector {1,2,3,...,n}
    ...
    for(int i=0; i<globalOptimum.second.size(); i++) {
        if(!i) cout << "{" << (globalOptimum.second)[i];
        else cout << "," << i;
    }
